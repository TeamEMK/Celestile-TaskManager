...
export const ORDER_PATTERN = /^[A-Z]{1,3}\d{1,6}$/;
...
export function splitOrderNumbers(value) {
  return String(value ?? '')
    .split(/[,;/&\n]+/)
    .map((s) => s.trim())
    .filter(Boolean);
...
export function normalizeOrderList(value) {
  return splitOrderNumbers(value).map(normalizeOrderNumber).filter(Boolean).join(', ');
}

// The parts that don't fit the shape — the input's own words, so a message
// can quote back what the person actually typed.
export function invalidOrderNumbers(value) {
  return splitOrderNumbers(value).filter((part) => !isValidOrderNumber(part));
}
