...
// Retry a transient Maytapi failure (network error, 429/5xx) with short
// backoff — same pattern as lib/quotationMaster.js's withRetry for the same
// class of error. Every send here used to be a single attempt with no retry,
// so one transient blip permanently dropped that WhatsApp notification (an
// approval request, a revision notice, a report) for good.
async function withRetry(fn) {
  ...
  for (let attempt = 0; attempt <= delays.length; attempt++) {
    try {
      ...
      if (!(code === 429 || code === 503 || code === 500) || attempt === delays.length) return result;
    } catch (err) {
      lastErr = err;
...
// Sent to the person an assigner just picked for an FMS step, so they know a
// task landed on their Dashboard without having to go look.
export function fmsStepAssignedMessage({ doerName, byName, fmsName, stepName, orderNo, plannedDate }) {
  return [
    `Hello ${doerName || 'there'},`,
    '',
    '📋 *FMS Step Assigned to You*',
    '',
    `*FMS:* ${fmsName || '—'}`,
    `*Step:* ${stepName || '—'}`,
    ...
    `*Assigned by:* ${byName || '—'}`,
    '',
    'It is on your Dashboard under FMS — mark it done there once finished.',
    '— Celestile-TaskManager',
