...
export function eaDailyReportMessage(dateStr, walkins, payments) {
  ...
  if (!payments?.length) {
    ...
    payments.forEach((e, i) => parts.push('', [
      ...
      `• Adv Paid: ${inr(e.advPaid)} | Bal: ${inr(e.balance)}`,
      `• Mode of Pay: ${e.modeOfPay || '-'}`,
      `• Till Date Received Total: ${inr(e.tillDateReceived)}`,
      `• Balance Target: ${inr(e.balanceTarget)}`,
      `• Executive: ${e.executive || '-'}`,
    ].join('\n')));
  }
