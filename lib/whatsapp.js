...
// A WhatsApp group JID (…@g.us) is already the exact ID Maytapi expects —
// stripping non-digits would mangle it, so pass it through untouched.
...
// Sent to the chosen approver when a doer marks an "Approval Required" task
// done — it now waits for their sign-off before it counts as complete.
export function approvalWaitingMessage({ approverName, doerName, description }) {
  return [
    `Hello ${approverName || 'there'},`,
    '',
    '⏳ *Task Awaiting Your Approval*',
    '',
    `*Completed by:* ${doerName || '—'}`,
    '',
    `*Task:* ${description || ''}`,
    '',
    'Please review and approve/reject in Approvals → Task Approvals.',
    '— Celestile-TaskManager',
  ].join('\n');
}
...
// Grouped FMS reminder — one message per user listing all their pending
// FMS steps (mirrors checklistReminderMessage's grouped format).
export function fmsReminderMessage(name, tasks) {
  const blocks = (tasks || []).map((t) => [
    `FMS - ${t.fmsName || ''}`,
    `Step - ${t.stepName || ''}`,
    `Plan - ${t.planValue || '-'}`,
  ].join('\n'));
  return `*${name || 'Hello'} - FMS Pending Task Summary*\n\n` + blocks.join('\n\n');
}
...
// Designer daily report — one message per designer (not one rolled-up
// message for the whole department), posted to the design WhatsApp group.
// One numbered block per client worked on today, with client no. / order
// no. / area / duration / software as bullets underneath.
export function designDailyReportMessage(name, entries) {
  const blocks = (entries || []).map((e, i) => [
    `${i + 1}. ${e.client || '-'} -`,
    `• ${e.clientNumber || '-'}`,
    `• ${e.orderNumber || '-'}`,
    `• ${e.areaName || '-'}`,
    `• ${Number(e.minutes) || 0} min`,
    `• ${e.software || '-'}`,
  ].join('\n'));
  return [name || '-', '', blocks.join('\n\n')].join('\n');
}

// Separate message for someone who hasn't filled today's daily task at all —
// sent on its own to their department's group, not folded into the summary
// above (which only ever lists people who have entries to report). Shared by
// the designer, sales and site-engineer reports.
export function dailyReportNotFilledMessage(name) {
  return `${name || '-'}\nDaily report not filled today ❌`;
}

// Sales daily report — same shape as the designer report above, but with the
// fields the Sales form actually captures: client number, type of task,
// purpose of task, area and (Hyderabad only) site location.
export function salesDailyReportMessage(name, entries) {
  const blocks = (entries || []).map((e, i) => [
    `${i + 1}. ${e.client || '-'} -`,
    `• ${e.clientNumber || '-'}`,
    `• ${e.taskType || '-'}`,
...
// Site Engineer daily report — order no. / site location / area / purpose of
// visit (with the Checks sub-type appended when the visit was a check) / kms
// travelled / duration.
export function siteDailyReportMessage(name, entries) {
  const blocks = (entries || []).map((e, i) => [
    `${i + 1}. ${e.client || '-'} -`,
    `• ${e.orderNumber || '-'}`,
    `• ${e.siteLocation || '-'}`,
    ...
    `• ${Number(e.kmsTravelled) || 0} km`,
    `• ${Number(e.minutes) || 0} min`,
  ].join('\n'));
  return [name || '-', '', blocks.join('\n\n')].join('\n');
}

// Executive Assistant evening report — ONE combined message (walk-ins +
// payments) for the day, sent to a single recipient (Vinay sir) rather than
// per-person into a department group like the reports above.
// `sales` is the month position from salesMonthSummary(): Till Date Received
// Total is the month's Adv Paid summed (not typed by anyone), Balance Target
// is monthly target − received.
export function eaDailyReportMessage(dateStr, walkins, payments, sales) {
  const inr = (v) => `₹${Number(v || 0).toLocaleString('en-IN')}`;
  const parts = [`*Daily Report — ${fmtDate(dateStr)}*`];

  parts.push('', '🚶 *Daily Walk-in Report*');
  if (!walkins?.length) {
    parts.push('No walk-ins reported today ❌');
  } else {
    walkins.forEach((e, i) => parts.push('', [
      `${i + 1}. ${e.client || '-'} (${e.clientNumber || '-'})`,
      `• Arc: ${e.arcName || '-'}${e.arcPhone ? ` (${e.arcPhone})` : ''}`,
      `• ${e.oldNewClient || '-'}`,
      `• Visits: ${Number(e.noOfVisits) || 0}`,
  ...
  parts.push('', '💰 *Sales Report (Payments)*');
  if (!payments?.length) {
    parts.push('No payments reported today ❌');
  } else {
    payments.forEach((e, i) => parts.push('', [
      `${i + 1}. ${e.client || '-'}`,
      `• Arc: ${e.arcName || '-'}`,
      `• Requirement: ${e.description || '-'}`,
      `• Order Value: ${inr(e.orderValue)}`,
      `• Advance Paid: ${inr(e.advPaid)} | Balance: ${inr(e.balance)}`,
      `• Mode of Payment: ${e.modeOfPay || '-'}`,
      `• Executive: ${e.executive || '-'}`,
    ].join('\n')));
  }

  // Bottom summary, mirroring the paper/Excel sheet: today's total received,
  // the month's running total, and what is left of the monthly target.
  const todayTotal = (payments || [])
    .reduce((s, e) => s + (Number(e.advPaid) || 0), 0);
  parts.push('',
    `*TOTAL: ${inr(todayTotal)}*`);
  if (sales) {
    parts.push(`*TILL DATE RECD TOTAL: ${inr(sales.received)}*`);
    parts.push(sales.target > 0
      ? `*BALANCE TARGET: ${inr(sales.target - sales.received)}*`
      : '*BALANCE TARGET:* — (monthly target not set)');
  }
...
// ── Help tickets ─────────────────────────────────────────────────────
// Sent to whoever a ticket lands on. The ticket id is in the message because
// it's what the list and the URL are keyed by (/help-tickets/<id>).
export function helpTicketAssignedMessage({ assigneeName, ticketId, subject, priority, category, dueDate, byName }) {
  return [
    `Hello ${assigneeName || 'there'},`,
    '',
    '🎫 *Help Ticket Assigned to You*',
    '',
    `*Ticket:* ${ticketId || '—'}`,
    `*Raised by:* ${byName || '—'}`,
    ...
    `*Issue:* ${subject || ''}`,
    '',
    'Open it in Help Tickets to comment or update the status.',
    '— Celestile-TaskManager',
  ].join('\n');
}

// Sent back to whoever raised the ticket once it is marked resolved — they're
// the only one who can say whether the problem actually went away.
export function helpTicketResolvedMessage({ raiserName, ticketId, subject, resolvedByName }) {
  return [
    `Hello ${raiserName || 'there'},`,
    '',
    ...
    `*Issue:* ${subject || ''}`,
    '',
    'If the problem is still there, reopen it from the ticket page.',
    '— Celestile-TaskManager',
  ].join('\n');
}
