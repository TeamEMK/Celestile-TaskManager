...
// Mirrors the "Department: design / Name: task (time)" format Vivek asked
// Vishal to post by hand every evening.
export function designDailyReportMessage(name, tasks) {
  const lines = (tasks || []).map((t, i) => {
    const mins = Number(t.minutes) || 0;
    const line = `${t.description || '-'} (${mins} min)`;
    return i === 0 ? `${name || '-'}: ${line}` : `      ${line}`;
  });
  return ['Department: design', ...lines].join('\n');
}
