...
// The read side (app/api/drive/[fileId]/route.js) caps what it will stream
// back at 50MB; nothing enforced a matching cap — or a mimetype allow-list —
// on the write side, so an arbitrary size/type could be written to the
// Shared Drive in the first place.
...
async function uploadDataUri(value, nameHint) {
  ...
  if (!ext) { console.error('[googleDrive] rejected upload — unsupported mimetype:', mimeType); return null; }
  if (buffer.length > MAX_UPLOAD_BYTES) { console.error('[googleDrive] rejected upload — too large:', buffer.length); return null; }
