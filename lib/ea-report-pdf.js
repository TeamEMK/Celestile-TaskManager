...
import { receivedAmount } from './dailyReport.js';
...
export async function generateEaReportPdf({ dateStr, walkins, payments, sales }) {
  ...
  // Column widths sum to CW (786): the table must span the sheet exactly.
  const payCols = [
    { label: 'DATE',            w: 50,  get: () => dateLabel },
    { label: "CLIENT'S NAME",   w: 100, get: (e) => e.client || '' },
    { label: 'ORDER NO.',       w: 66,  get: (e) => e.orderNumber || '' },
    { label: 'ARCHITECT',       w: 84,  get: (e) => e.arcName || '' },
    { label: 'REQUIREMENTS',    w: 100, get: (e) => e.description || '' },
    { label: 'ORDER VALUE',     w: 72,  right: true, get: (e) => money(e.orderValue) },
    { label: 'ADVANCE PAID',    w: 72,  right: true, get: (e) => money(e.advPaid) },
    { label: 'RECD TODAY',      w: 72,  right: true, get: (e) => money(receivedAmount(e)) },
    { label: 'BALANCE',         w: 72,  right: true, get: (e) => money(e.balance) },
    { label: 'MODE OF PAY',     w: 50,  get: (e) => e.modeOfPay || '' },
    { label: 'EXECUTIVE',       w: 48,  get: (e) => e.executive || '' },
  ];
  drawTable('SALES REPORT — PAYMENTS', payCols, payments || []);

  // TOTAL is the day's receipts (Recd Today column); TILL DATE RECD TOTAL is
  // the month's (received-before-app opening + every entry this month, see
  // salesMonthSummary), so the two lines add up the way the Excel does.
  const todayTotal = (payments || []).reduce((s, e) => s + receivedAmount(e), 0);
  const valueW = 110;
