...
//
// The range passed here is what Sheets uses to locate "the table" to append
// after — anchoring on just `A1` restricts that search to column A alone, so
// a row with data only in later columns (e.g. a stray summary/label row
// above the real header, with a blank col A) reads as the end of the table
// and the new row lands there instead of below the sheet's real last row.
// Spanning the full column width makes it consider every column.
