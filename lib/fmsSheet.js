...
export async function getMyFmsPendingRows({ userId, userName, isAdmin, branch }) {
  const sheets = await getFmsSheetsForUser(userId, isAdmin);
  if (!sheets.length) return [];

  // Admin/HOD/Factory see every branch (branchScopeFor returns null for
  // them); a Bangalore/Hyderabad user gets cut to their own branch's rows,
  // same rule as Live Tracking.
  const branchScope = branchScopeFor({ roles: isAdmin ? ['Admin'] : [], branch });
