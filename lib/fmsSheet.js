...
// Append one new row to the end of a tab — Sheets finds the next empty row
// itself, no row-number math needed (used by the FMS intake form).
export async function appendRow(sheetUrlOrId, tabName, rowValues) {
  const sheets = getSheetsRW();
  const spreadsheetId = extractSpreadsheetId(sheetUrlOrId);
...
export async function saveIntakeFields(fmsId, fields) {
  ...
  for (let i = 0; i < rows.length; i++) {
    const f = rows[i];
    await pool.query(
      'INSERT INTO fms_intake_fields (id, fms_id, field_label, col_letter, field_type, dropdown_options, required, sort_order) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
      [
...
// Count data rows in a sheet's first column, past the header — a cheap
// "Total entries" proxy for the FMS Tracker list (used instead of pulling
// every FMS's full grid just to show a number).
export async function countSheetEntries(sheet) {
  const values = await fetchRange(sheet.sheet_id, `${sheet.sheet_name}!A:A`);
  const headerIdx = (sheet.header_row || 1) - 1;
  return values.slice(headerIdx + 1).filter((r) => (r[0] || '').toString().trim()).length;
}

// FMS list + stats for the FMS Tracker page: total steps, total entries,
// and the resolved Process Coordinator name.
export async function getFmsSheetsWithStats(userId, isAdmin) {
  ...
  return Promise.all(sheets.map(async (sheet) => {
    const [steps, totalEntries] = await Promise.all([
      getFullSteps(sheet.id),
      countSheetEntries(sheet).catch(() => 0),
    ]);
    return {
      ...sheet,
...
// for non-admins via the step's configured doer_name_col.
export async function getPendingRowsForStep({ sheet, step, userName, isAdmin }) {
  ...
  const maxIdx = Math.max(planIdx, actualIdx, doerIdx, ...(showCols.length ? showCols : [0]));
...
// monitoring overview, not filtered to one doer/step). Reuses
// getPendingRowsForStep with isAdmin:true so no doer filter is applied.
export async function getPendingAcrossSteps(fmsId) {
  ...
  for (const step of steps) {
    let result;
    try { result = await getPendingRowsForStep({ sheet, step, userName: '', isAdmin: true }); } catch { continue; }
...
// Full live grid for FMS Tracker's "FMS View" tab — the whole used range of
// the tab (bare tab name as the range fetches everything Sheets has for it).
export async function getSheetGridData(sheet) {
  const rows = await fetchRange(sheet.sheet_id, sheet.sheet_name);
  return { rows, headerRow: sheet.header_row || 1 };
}

// Validate required fields, then append one new row built from {fieldId: value}
// pairs, placing each value at its configured column and leaving the rest blank.
export async function submitIntakeRow(sheet, fields, values) {
  ...
  const maxIdx = Math.max(0, ...fields.map((f) => colToIdx(f.col_letter)));
  const row = new Array(maxIdx + 1).fill('');
  fields.forEach((f) => {
    const idx = colToIdx(f.col_letter);
    if (idx >= 0) row[idx] = values[f.id] ?? '';
  });
  await appendRow(sheet.sheet_id, sheet.sheet_name, row);
}
...
export async function getMyFmsPendingRows({ userId, userName, isAdmin }) {
  ...
  for (const sheet of sheets) {
    ...
    for (const step of relevantSteps) {
      let result;
      try {
        result = await getPendingRowsForStep({ sheet, step, userName, isAdmin });
      } catch { continue; }
