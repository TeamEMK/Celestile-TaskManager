...
import { isDateOffsetField, computeDateOffset } from '@/lib/dateOffset';
import { sendWhatsApp, isWhatsappConfigured, fmsStepAssignedMessage } from '@/lib/whatsapp';
import { isHttpUrl } from '@/lib/url';
import { branchScopeFor, rowInBranchScope } from '@/lib/liveTrackingView';
...
// step_id -> [{ user_id, name }] for one of the two step↔user link tables:
// fms_step_doers (who may do the step) or fms_step_assigners (who picks the
// doer at run time — see isAssignedStep).
const STEP_USER_TABLES = { doers: 'fms_step_doers', assigners: 'fms_step_assigners' };
async function stepUsersMap(kind, stepIds) {
  if (!stepIds.length) return {};
  // No WHERE ... IN — the Sheets SQL engine can't parse it; filter in JS.
  const wanted = new Set(stepIds.map(String));
  const [allRows] = await pool.query(`SELECT step_id, user_id FROM ${STEP_USER_TABLES[kind]}`);
...
// Design FMS: whoever a step's config names, the people who pick doers in
// the Design flow are these three (asked 2026-09-17). Matched on the first
// word of the user's name, so "Jibin K" and "jibin" both count. Only steps
// that already have assigners are affected — this narrows the list, it never
// switches a fixed-doer step into assigned mode. Admin/HOD can still assign
// (the assign route allows them regardless of the list).
const DESIGN_FMS_ASSIGNERS = ['jibin', 'umesh', 'vivek'];
const isDesignFms = (sheet) => /design/i.test(String(sheet?.fms_name || sheet?.sheet_name || ''));

async function designAssigners() {
  const [users] = await pool.query('SELECT id, name FROM users');
  return (users || [])
    .filter((u) => DESIGN_FMS_ASSIGNERS.includes(String(u.name || '').trim().split(/\s+/)[0].toLowerCase()))
...
export async function getFullSteps(fmsId) {
  ...
  const [doersMap, assignersMap, extraMap, sheet] = await Promise.all([
    stepUsersMap('doers', stepIds), stepUsersMap('assigners', stepIds), stepExtraRowsMap(stepIds),
    getFmsSheet(fmsId),
  ]);
  const forced = isDesignFms(sheet) ? await designAssigners() : null;
  return steps.map((s) => ({
    ...s,
    doers: doersMap[s.id] || [],
    assigners: (forced?.length && (assignersMap[s.id] || []).length) ? forced : (assignersMap[s.id] || []),
...
export async function updateFmsSheet(id, { fmsName, sheetName, sheetId, headerRow, steps, processCoordinatorId }) {
  ...
  for (const s of oldSteps) {
    ...
    await pool.query('DELETE FROM fms_step_assigners WHERE step_id = ?', [s.id]);
...
async function writeSteps(fmsId, steps) {
  for (let i = 0; i < steps.length; i++) {
    ...
    for (const uid of s.assigners || []) {
      await pool.query('INSERT INTO fms_step_assigners (step_id, user_id) VALUES (?, ?)', [stepId, String(uid)]);
    }
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    await pool.query(
      ...
      [
        ...
        // Same for a computed Tentative Date.
        (!f.auto_fill && !isDateOffsetField(f) && (f.unique_value === 1 || f.unique_value === true || f.unique_value === '1')) ? 1 : 0,
      ]
...
async function getFmsSheetsForUser(userId, isAdmin, canSubmitIntake = false) {
  ...
  // A flow is "mine" if I do a step in it OR I pick the doer for one.
  const [myDoerRows] = await pool.query('SELECT step_id FROM fms_step_doers WHERE user_id = ?', [String(userId)]);
  const [myAssignerRows] = await pool.query('SELECT step_id FROM fms_step_assigners WHERE user_id = ?', [String(userId)]);
  const myStepIds = new Set([...myDoerRows, ...myAssignerRows].map((r) => r.step_id));
...
// "Assigned" mode: the doer isn't fixed in config but picked per row, by one
// of the step's assigners, once the previous step is done. The step's doers
// list is then the pool to pick from, and the Doer Name column is where the
// pick lands — so the mode only works with that column configured.
export function isAssignedStep(step) {
  return !!(step?.assigners?.length && step?.doer_name_col);
}

// Config-time guard for the editor: assigners without a Doer Name column
// would silently behave like a normal fixed-doer step.
export function validateStepsConfig(steps) {
  for (let i = 0; i < (steps || []).length; i++) {
    const s = steps[i];
    if ((s.assigners || []).length && !(s.doerNameCol || '').trim()) {
      return `Step ${i + 1} (${s.stepName || 'unnamed'}): a Doer Name column is required when someone is set to choose the doer.`;
    }
    if ((s.assigners || []).length && !(s.doers || []).length) {
      return `Step ${i + 1} (${s.stepName || 'unnamed'}): add at least one Step Doer — that list is what the doer gets picked from.`;
    }
    // Passed straight to window.open() (lib/fmsOpenUrl.js / useTaskCompletion.js)
    // — a non-http(s) scheme (e.g. `javascript:...`) must never be stored here.
    if ((s.openUrl || '').trim() && !isHttpUrl(s.openUrl)) {
      return `Step ${i + 1} (${s.stepName || 'unnamed'}): Open Page URL must start with http:// or https://.`;
    }
  }
  return '';
}
...
// "mine" if I can do it or I pick who does it.
export async function getStepsForTaskView(fmsId, userId, isAdmin) {
  const steps = await getFullSteps(fmsId);
  return steps.map((s) => {
    const isDoer = s.doers.some((d) => String(d.user_id) === String(userId));
    const isAssigner = s.assigners.some((a) => String(a.user_id) === String(userId));
    return {
      ...s,
      assigned: isAssignedStep(s),
      isDoer, isAssigner,
      isMyStep: isAdmin || isDoer || isAssigner,
    };
  });
}
...
//
// `opts.isAssigner` — the caller picks doers for this step. Matters only for
// assigned-mode steps (isAssignedStep): there a blank Doer Name cell means
// "nobody has been given this yet", so the row is shown to the assigners as
// an "assign" task and to nobody else; once a name is in the cell only that
// person (and admins) see it.
function computePendingRows(allRows, sheet, step, userName, isAdmin, opts = {}) {
  ...
  const assigned  = isAssignedStep(step) && doerIdx >= 0;
  ...
  dataRows.forEach((row, i) => {
    ...
    // Same branch cut as Live Tracking (lib/liveTrackingView.js): a Bangalore
    // user never sees a Hyderabad order's task, whatever step or doer it's on.
    // Cut before totalPending so a branch user's counts don't include work
    // that was never shown to them.
    if (!isAdmin && opts.branchScope && !rowInBranchScope(row, { orderIdx }, opts.branchScope)) return;
    totalPending++;

    const rowDoer = doerIdx >= 0 ? (row[doerIdx] || '').trim() : '';
    const isMine = rowDoer.toLowerCase() === myName;
    if (isMine) assignedToMe++;
    // The Doer Name column is written when a step is marked done (see
    // writeStepDone), so on a pending row it is usually still BLANK — a
    // blank means "nobody has claimed this yet", not "not yours". Only a
    // row that names someone else is hidden; hiding blank rows too made
    // every pending task of a multi-doer step (Design O2D) vanish for every
    // non-admin doer.
    const needsAssign = assigned && !rowDoer;
    if (assigned) {
      if (!isAdmin && (needsAssign ? !opts.isAssigner : !isMine)) return;
    } else if (applyDoerFilter && rowDoer && !isMine) return;
    ...
    matchedRows.push({
      ...
      rowDoerName: rowDoer, isMine, assigned, needsAssign, data: rowData,
...
export async function getPendingAcrossSteps(fmsId) {
  ...
  for (const step of steps) {
    ...
    result.rows.forEach((row) => {
      items.push({
        stepId: step.id, stepName: step.step_name,
        rowNumber: row.sheetRowNumber,
        orderNo: row.orderNo || '',
        doer: row.rowDoerName || (row.assigned ? 'Unassigned' : step.doers.map((d) => d.name).join(', ')) || '—',
        // Assigned-mode rows carry what the PC view needs to (re)assign them.
        assigned: !!row.assigned, needsAssign: !!row.needsAssign,
...
export async function submitIntakeRow(sheet, fields, rawValues, { userName } = {}) {
  ...
  // Tentative Date fields — one left blank on the form is filled in from its
  // start date + days (lib/dateOffset). Goes through valueOf so the
  // auto-filled Timestamp can be the start date, and runs before the
  // required check so leaving it blank is always allowed. A date the person
  // did pick is kept as typed.
  for (const [i, f] of fields.entries()) {
    if (!visible[i] || !isDateOffsetField(f) || String(values[f.id] ?? '').trim()) continue;
    values[f.id] = computeDateOffset(f, fields, valueOf).iso || '';
  }

  const missing = fields.filter((f, i) => visible[i] && !f.auto_fill && !isDateOffsetField(f) && f.required && !String(values[f.id] ?? '').trim());
  if (missing.length) {
...
// Assigned-mode steps (isAssignedStep): name the person who will do one
// pending row. Writes their name into the step's Doer Name column — the same
// cell writeStepDone fills on completion — so from here on computePendingRows
// shows the row to them alone. Calling it again simply reassigns.
export async function assignStepDoer({ sheet, step, rowNumber, userId, assignedBy }) {
  const inPool = (step.doers || []).find((d) => String(d.user_id) === String(userId));
  if (!inPool) {
    const e = new Error("That person is not in this step's doer list — add them under Step Doer(s) in the FMS editor first");
    e.status = 400; throw e;
  }
  const [urows] = await pool.query('SELECT id, name, phone FROM users WHERE id = ?', [String(userId)]);
  const target = urows[0];
  const doerName = (target?.name || inPool.name || '').trim();
  if (!doerName) { const e = new Error('That user has no name on file'); e.status = 400; throw e; }

  await writeCell(sheet.sheet_id, sheet.sheet_name, step.doer_name_col, rowNumber, doerName);

  // Best-effort WhatsApp to the new doer. Order no / planned date are read
  // off the row purely for the message, so a read failure must not undo a
  // write that already succeeded.
  if (isWhatsappConfigured() && target?.phone) {
    try {
      const headerRowIdx = (sheet.header_row || 1) - 1;
      const [headers = []] = await fetchRange(sheet.sheet_id, `${sheet.sheet_name}!${headerRowIdx + 1}:${headerRowIdx + 1}`);
      const [row = []] = await fetchRange(sheet.sheet_id, `${sheet.sheet_name}!${rowNumber}:${rowNumber}`);
      const orderIdx = headers.findIndex((h) => /order\s*(no|number|#)/i.test(String(h || '')));
      const planIdx = colToIdx(step.plan_col);
      await sendWhatsApp(target.phone, fmsStepAssignedMessage({
        doerName, byName: assignedBy?.name || '',
        fmsName: sheet.fms_name || sheet.sheet_name, stepName: step.step_name,
        orderNo: orderIdx >= 0 ? String(row[orderIdx] || '').trim() : '',
        plannedDate: planIdx >= 0 ? String(row[planIdx] || '').trim() : '',
      }));
    } catch (e) { console.error('[fms assign] WhatsApp notify failed:', e.message); }
  }
  return { doerName };
}
...
export async function getMyFmsPendingRows({ userId, userName, isAdmin, branch, department }) {
  const sheets = await getFmsSheetsForUser(userId, isAdmin);
  if (!sheets.length) return [];

  // Admin/HOD/Factory/Designer see every branch (branchScopeFor returns null
  // for them); a Bangalore/Hyderabad user outside those gets cut to their own
  // branch's rows, same rule as Live Tracking.
  const branchScope = branchScopeFor({ roles: isAdmin ? ['Admin'] : [], branch, department });
  ...
  const perSheet = await Promise.all(sheets.map(async (sheet) => {
    // A failure here used to vanish silently — an FMS whose sheet the
    // service account can't read, or whose tab was renamed, simply had no
    // tasks on the Dashboard with nothing in the logs to say why.
    const label = sheet.fms_name || sheet.sheet_name || sheet.id;
    let steps;
    try {
      steps = await getStepsForTaskView(sheet.id, userId, isAdmin);
    } catch (err) { console.error(`[fms] "${label}": steps unreadable —`, err.message); return []; }
    const relevantSteps = isAdmin ? steps : steps.filter((s) => s.isMyStep);
    if (!relevantSteps.length) return [];

    let allRows;
    try { allRows = await fetchRange(sheet.sheet_id, `${sheet.sheet_name}!A:${lastColForSteps(relevantSteps)}`); }
    catch (err) { console.error(`[fms] "${label}": sheet read failed (tab "${sheet.sheet_name}") —`, err.message); return []; }

    const items = [];
    for (const step of relevantSteps) {
      const result = computePendingRows(allRows, sheet, step, userName, isAdmin, { isAssigner: step.isAssigner, branchScope });
      ...
      result.rows.forEach((row) => {
        ...
        items.push({
          ...
          orderNo: row.orderNo || '',
          doer: row.rowDoerName || (row.assigned ? 'Unassigned' : step.doers.map((d) => d.name).join(', ')) || '—',
          // An unassigned row of an assigned-mode step is an "assign" task,
          // not a "do" task — the UI swaps the Done button for Assign, and
          // admins/assigners can also reassign a row that already has a name.
          needsAssign: !!row.needsAssign,
          canAssign: !!row.assigned && (isAdmin || !!step.isAssigner),
          planValue: row.planValue,
