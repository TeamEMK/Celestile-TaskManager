...
export async function createFmsSheet({ fmsName, sheetName, sheetId, headerRow, createdBy, steps }) {
  await ensureSchema();
  const id = 'FMS' + Date.now();
  await pool.query(
    'INSERT INTO fms_sheets (id, fms_name, sheet_name, sheet_id, header_row, created_by, created_at) VALUES (?, ?, ?, ?, ?, ?, NOW())',
    [id, fmsName || sheetName || '', sheetName || '', sheetId || '', headerRow || 1, createdBy || null]
  );
  await writeSteps(id, steps || []);
  return id;
}

export async function updateFmsSheet(id, { fmsName, sheetName, sheetId, headerRow, steps }) {
  await ensureSchema();
  await pool.query(
    'UPDATE fms_sheets SET fms_name = ?, sheet_name = ?, sheet_id = ?, header_row = ? WHERE id = ?',
    [fmsName || sheetName || '', sheetName || '', sheetId || '', headerRow || 1, id]
  );
