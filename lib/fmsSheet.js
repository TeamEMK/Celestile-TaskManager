...
function getSheetsRW() {
  ...
  if (!client_email || !private_key) throw new Error('Google credentials not configured (GOOGLE_SERVICE_ACCOUNT_EMAIL / GOOGLE_PRIVATE_KEY)');
...
async function rawSteps(fmsId) {
  await ensureSchema();
  const [rows] = await pool.query('SELECT * FROM fms_steps WHERE fms_id = ? ORDER BY step_order ASC', [fmsId]);
  return rows;
}
...
export async function updateFmsSheet(id, { fmsName, sheetName, sheetId, headerRow, steps }) {
  ...
  for (const s of oldSteps) {
    ...
    await pool.query('DELETE FROM fms_steps WHERE id = ?', [s.id]);
  }
  await writeSteps(id, steps || []);
}
...
export async function deleteFmsSheet(id) {
  ...
  await pool.query('DELETE FROM fms_steps WHERE fms_id = ?', [id]);
...
export async function getFmsSheetsForUser(userId, isAdmin) {
  ...
  const [allSteps] = await pool.query('SELECT id, fms_id FROM fms_steps');
