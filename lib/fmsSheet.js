...
export async function saveIntakeFields(fmsId, fields) {
  ...
  for (let i = 0; i < rows.length; i++) {
    const f = rows[i];
    await pool.query(
      'INSERT INTO fms_intake_fields (id, fms_id, field_label, col_letter, field_type, dropdown_options, required, sort_order, auto_fill, auto_fill_value, depends_on, depends_value) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
      [
...
export async function getFmsSheetsForUser(userId, isAdmin, canSubmitIntake = false) {
  ...
  if (canSubmitIntake) {
    const [intakeRows] = await pool.query('SELECT DISTINCT fms_id FROM fms_intake_fields');
