...
export async function saveIntakeSheetConfig(fmsId, { sheetId, sheetName, headerRow }) {
  await ensureSchema();
  await pool.query(
    'UPDATE fms_sheets SET intake_sheet_id = ?, intake_sheet_name = ?, intake_header_row = ? WHERE id = ?',
    [sheetId || '', sheetName || '', headerRow || null, fmsId]
  );
}
