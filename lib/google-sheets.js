...
// Legacy "export DB to a human-readable Sheet" feature (app/api/sync-sheets),
// separate from the actual Sheets-DB engine in lib/sheets-client.js. The id
// was hardcoded and shared by every environment using the same service
// account, so a staging run of /api/sync-sheets silently overwrote
// production's export tabs. SYNC_SHEET_ID must be set per deployment now;
// leaving it unset disables the sync rather than falling back to a shared id.
const SPREADSHEET_ID = process.env.SYNC_SHEET_ID || '';
...
async function syncTab(title, headers, rows) {
  if (!SPREADSHEET_ID) { console.error('[Sheets] sync disabled — SYNC_SHEET_ID not set'); return; }
