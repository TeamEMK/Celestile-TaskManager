...
function tick() {
  ...
  if (now === _lastMs) {
    ...
    // have handed out. Same overflow guard as the same-millisecond branch
    // above — without it, a sustained backward step combined with enough
    // calls during that window could push _seq past SEQ_MAX and emit an id
    // longer than the fixed width this scheme promises.
    now = _lastMs;
    _seq += 1;
    if (_seq >= SEQ_MAX) {
      while (Date.now() <= now) { /* spin */ }
