...
function compareFixingItems(prev, curr) {
  ...
  for (let i = 0; i < maxLen; i++) {
    const p = prev[i], c = curr[i];
    if (p && c) {
      const lbl = c.desc || c.col1 || ('Fixing ' + (i + 1));
      if ((+p.qty   || 0) !== (+c.qty   || 0)) out.push('  • ' + _truncate(lbl, 22) + ': qty ' + (p.qty || 0) + '→' + (c.qty || 0));
      if ((+p.price || 0) !== (+c.price || 0)) out.push('  • ' + _truncate(lbl, 22) + ': rate ₹' + (p.price || 0) + '→₹' + (c.price || 0));
    }
  }
  return out;
}
