...
// GOOGLE_PRIVATE_KEY) OR a local, gitignored credentials.json file.
...
export const SCHEMAS = {
  users: ['id', 'name', 'email', 'phone', 'department', 'roles', 'active', 'password_hash', 'picture', 'force_logout_after', 'created_at'],
  delegations: ['id', 'description', 'doer_id', 'doer', 'delegated_by', 'due_date', 'client', 'status', 'type', 'priority', 'approval', 'url', 'remarks', 'revise_action', 'transferred_by', 'transferred_from', 'created_at', 'completed_at'],
  masters: ['id', 'task', 'assigned_to', 'frequency', 'start_date', 'created_at'],
  holidays: ['id', 'date', 'name', 'type'],
  fms: ['id', 'client_name', 'platforms', 'mobile', 'doer', 'created_at'],
  fms_steps: ['fms_id', 'step_index', 'planned', 'actual'],
  ...
  daily_tasks: ['id', 'entry_date', 'doer_id', 'doer', 'client', 'department', 'description', 'minutes', 'created_at'],
...
export const PRIMARY_KEYS = {
  ...
  fms: ['id'], fms_steps: ['fms_id', 'step_index'], profile: ['user_id'],
...
function sheetsApi() {
  ...
  if (!creds) throw new Error('Google credentials not configured (env vars or credentials.json)');
...
function rowsToObjects(table, values) {
  ...
  for (let i = 1; i < values.length; i++) {
    ...
    headers.forEach((h, ci) => {
      ...
      if (ints.includes(h)) v = v === '' ? null : Number(v);
      obj[h] = v;
    });
    out.push(obj);
  }
