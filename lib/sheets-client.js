...
export const SCHEMAS = {
  ...
  // Who picks the doer for a step at run time (see lib/fmsSheet.js — a step
  // with any assigner is in "assigned" mode).
  fms_step_assigners: ['step_id', 'user_id'],
  ...
  daily_tasks: ['id', 'entry_date', 'doer_id', 'doer', 'client', 'department', 'description', 'minutes', 'created_at', 'order_number', 'area_name', 'task_type', 'software', 'revision',
    ...
    // Deliberately NOT in the numeric list below: a blank cell must stay
    // blank (pre-2026-09-17 rows fall back to adv_paid — see receivedAmount()
    // in lib/dailyReport.js), not read as 0.
    'received_today'],
...
export const PRIMARY_KEYS = {
  ...
  fms_step_assigners: ['step_id', 'user_id'], fms_extra_rows: ['id'], fms_intake_fields: ['id'], live_trackers: ['id'], profile: ['user_id'],
...
const INT_COLUMNS = {
  users: ['active'],
  daily_tasks: ['minutes', 'kms_travelled', 'order_value', 'adv_paid', 'balance', 'till_date_received', 'balance_target'],
