...
// Month's Sales Payment position: everything received so far this month
// (sum of Adv Paid across every Sales Payment row, whole month) against the
// monthly target stored in app_config under sales_target_<YYYY-MM>. Balance
// Target is simply target − received. Used by the EA report and by the
// /api/daily-tasks/sales-target route the Payments form reads and writes.
//
...
export async function salesMonthSummary(month = istDateStr().slice(0, 7)) {
  await ensureSchema();
  const [all] = await pool.query('SELECT entry_date, department, adv_paid FROM daily_tasks');
  const received = (all || [])
    .filter((r) => r.department === 'Sales Payment'
      && String(r.entry_date || '').slice(0, 7) === month)
    .reduce((s, r) => s + (Number(r.adv_paid) || 0), 0);
  const [cfg] = await pool.query(
    'SELECT `value` FROM app_config WHERE `key` = ?', [`sales_target_${month}`]);
  const target = Number(cfg?.[0]?.value) || 0;
  return { month, received, target, balanceTarget: target - received };
}
