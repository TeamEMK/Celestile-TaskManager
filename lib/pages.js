...
// Executive Assistant — the one non-admin department that may add users
// (asked 2026-09-17). Same spelling test as deptToFormType() in
// app/daily-task/DailyTaskClient.jsx.
export function isEaDept(dept) {
  const d = String(dept || '').toLowerCase().trim();
  return d === 'ea' || d === 'executive assistant';
}

// Who may open Users & Access and add a user: Admin/HOD, plus the EA. Edit,
// delete, passwords and the access matrix stay Admin/HOD only.
export function canManageUsers(roles, department) {
  return isAdminRoles(roles) || isEaDept(department);
}
