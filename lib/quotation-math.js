...
export function fixAmount(r) {
  // Negative price/qty (a stray minus sign) would silently reduce the total
  // instead of erroring — clamp to 0 rather than let it subtract.
  const price = Math.max(parseFloat(r.price) || 0, 0);
  const qty = Math.max(parseFloat(r.qty) || 0, 0);
