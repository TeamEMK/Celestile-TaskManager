...
import { CELESTILE_LOGO, CELESTILE_LOGO_FULL, CELESTILE_MARK_RED } from './celestile-logo';
import { MONTHS, roundDim6, fixAmount } from './quotation-math.js';
...
function stoneQty(r) {
  if (r.module) {
    const wt = Math.max(parseFloat(r.sizeWt || String(r.size || '').split(/[x×*\s]+/i)[0]) || 0, 0);
    const ht = Math.max(parseFloat(r.sizeHt || String(r.size || '').split(/[x×*\s]+/i)[1]) || 0, 0);
    return (roundDim6(wt) * roundDim6(ht)) / 144;
  }
  // A negative qty (stray minus sign) would silently reduce the line/grand
  // total instead of erroring — clamp to 0 rather than let it subtract.
  return Math.max(parseFloat(r.qty) || 0, 0);
}

// Bangalore charges/GST breakdown — mirrors compute() in app/quotation/BangaloreForm.jsx
export function computeBangaloreCharges(stone, totalsConfig) {
  const rowData = stone.map((r) => {
    const price = Math.max(parseFloat(r.price) || 0, 0), qty = stoneQty(r);
...
export function computeHyderabadCharges(stone, fixing, q) {
  let stoneSum = 0, grossGst = 0;
  stone.forEach((r) => {
    const p = Math.max(parseFloat(r.price) || 0, 0), qty = stoneQty(r), gst = parseFloat(r.gst) || 0;
    stoneSum += p * qty; grossGst += p * qty * gst / 100;
  });
  // Clamped to [0, 100] — same guard the Bangalore path already applies to
  // its discount. An out-of-range value (a fat-fingered "1000" instead of
  // "10.00") used to drive netStone/netGst negative with no server-side check.
  let discPct = parseFloat(q.discount_pct) || 0;
  ...
  // GST applies to everything except Installation Charges — mirrors compute() in HyderabadForm.jsx.
  const fixGst = fixSum * DEFAULT_GST / 100;
  const packingGst = packing * DEFAULT_GST / 100;
  const stoneDesignGst = netGst + designGst;
  const swg = netStone + designFees + stoneDesignGst;
  const totalGst = stoneDesignGst + fixGst + packingGst;
...
export async function generateQuotationPdf(q) {
  ...
  // Hyderabad's header uses this full lockup (icon + wordmark, client-supplied
  // grey backing) instead of the plain gold medallion — see the header block below.
  let logoFullImg = null;
  try {
    logoFullImg = await doc.embedJpg(b64ToBytes(CELESTILE_LOGO_FULL.split(',')[1]));
  } catch { logoFullImg = null; }
  let redMarkImg = null;
  try {
  ...
  if (isHydBranch && logoFullImg) {
    // Hyderabad: the client-supplied full lockup (icon + "celes'tile" wordmark,
    // on its own grey backing) replaces the plain gold medallion + separate text.
    const logoH = HEADER_H - 24;
    const s = logoH / logoFullImg.height;
    const iw = logoFullImg.width * s, ih = logoFullImg.height * s;
    pg.drawImage(logoFullImg, { x: leftX, y: hMid - ih / 2, width: iw, height: ih });
  } else {
    const logoR = 24;
    if (logoImg) {
