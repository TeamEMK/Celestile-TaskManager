...
export async function generateQuotationPdf(q) {
  ...
  function t(str, x, yOff, sz, font, color) {
    ...
    pg.drawText(String(str).slice(0, 100), { x, y: ay(yOff), size: sz ?? 8.5, font: font ?? F, color: color ?? DARK });
  }
  ...
  function infoBlock(title, pairs, xOrig) {
    ...
    for (const [lbl, val] of pairs) {
      if (!val) continue;
      pg.drawText(lbl + ':', { x: xOrig, y: PH - PM - ly, size: 7.5, font: F, color: LGRAY });
      pg.drawText(String(val).slice(0, 52), { x: xOrig + 60, y: PH - PM - ly, size: 7.5, font: F, color: DARK });
      ly += 11;
    }
    return ly - startYt;
  }
  ...
  function drawTable(title, headers, colW, dataRows) {
    ...
    dataRows.forEach((row, ri) => {
      ...
      let dx = PM + 4;
      row.forEach((cell, ci) => {
        const s  = String(cell ?? '').slice(0, 48);
        const sz = 7.5;
