/**
 * Hybrid data store:
 *  - If DB_HOST env var is set → uses MySQL (lib/db.js).
...
/* =====================================================================
   PUBLIC API — auto-routes to JSON or MySQL based on env
   ===================================================================== */
...
/* =====================================================================
   MYSQL IMPLEMENTATION (loaded lazily so missing DB env doesn't crash dev)
   ===================================================================== */

async function readStoreDb() {
  const mod = USE_POSTGRES ? './store-postgres.js' : './store-mysql.js';
  const { readStoreDb: impl } = await import(mod);
  return impl();
}
async function writeStoreDb(data) {
  const mod = USE_POSTGRES ? './store-postgres.js' : './store-mysql.js';
  const { writeStoreDb: impl } = await import(mod);
  return impl(data);
}
