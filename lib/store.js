 ...
 *  - If DB_HOST env var is set → uses MySQL (lib/store-mysql.js).
 *  - Otherwise → reads/writes data/store.json (good for local dev + offline work).
 *
...
// Static imports — required for Vercel (dynamic imports are not supported)
import { readStoreDb as readStoreDbPg, writeStoreDb as writeStoreDbPg } from './store-postgres.js';
import { readStoreDb as readStoreDbMysql, writeStoreDb as writeStoreDbMysql } from './store-mysql.js';
import { USE_SHEETS } from './db.js';

// Google Sheets mode reuses the MySQL store impl — its queries run through the
// sheets-backed `pool` in lib/db.js, so the same code path works unchanged.
const USE_POSTGRES = !USE_SHEETS && !!(process.env.POSTGRES_URL || process.env.DATABASE_URL);
const USE_MYSQL    = !USE_SHEETS && !USE_POSTGRES && !!process.env.DB_HOST;
const USE_DB       = USE_POSTGRES || USE_MYSQL || USE_SHEETS;
...
async function readStoreDb() {
  return USE_POSTGRES ? readStoreDbPg() : readStoreDbMysql();
}

async function writeStoreDb(data) {
  return USE_POSTGRES ? writeStoreDbPg(data) : writeStoreDbMysql(data);
}
...
async function seedStoreJson() {
  ...
  const initial = {
    users, delegations, masters, holidays,
...
// Tasks awaiting admin approval (approval = 'Approval Required' && status = 'pending'),
// org-wide (`total`, `byBranch` — keyed off the doer's user profile branch) plus the
// current user's own count (`mine`), for the dashboard KPI card.
export function computePendingApprovals(store, { currentUserId } = {}) {
  const all = (store.delegations || []).filter(
    (d) => d.approval === 'Approval Required' && (d.status || 'pending') === 'pending'
  );
  const mine = currentUserId ? all.filter((d) => d.doerId === currentUserId).length : 0;
  ...
  return { total: all.length, mine, byBranch };
}
