...
export async function getLiveTrackerData(tracker) {
  ...
  let fileLinks = {}, fileLinkError = '';
  if (paths.length) {
    try {
      ({ links: fileLinks, error: fileLinkError } = await resolveAppSheetFiles(tracker.sheet_id, paths));
    } catch (err) {
      fileLinkError = err.message;
    }
  }

  return { headers, rows, fileLinks, fileLinkError };
}
