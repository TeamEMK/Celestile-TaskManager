...
// Rows whose branch cell is blank stay visible to everyone: the sheet not
// saying which branch a job belongs to is no reason to hide it from the branch
// that may well be doing it.
export function rowInBranchScope(row, branchIdx, scope) {
  if (!scope || !scope.length) return true;
  if (branchIdx == null || branchIdx < 0) return true;
  const b = branchOf(row?.[branchIdx]);
  return !b || scope.includes(b);
}
...
// Any label the sheet invents ("Regular") still gets a badge — a neutral one,
// so only the ones that mean urgency carry colour. '' for an empty cell.
...
// the first cell.
//
// `other` catches the labels a sheet invents — these trackers mostly say
// "High"/"Regular" and never touch Medium/Low, so without it nearly every
// open row would come out white and the colouring would look broken. Only a
// genuinely empty priority cell stays white.
export const ROW_TINT = {
  done:   { cls: 'row-done',   rail: '#10B981' },
  High:   { cls: 'row-high',   rail: '#EF4444' },
  Medium: { cls: 'row-medium', rail: '#F59E0B' },
  Low:    { cls: 'row-low',    rail: '#3B82F6' },
  other:  { cls: 'row-other',  rail: '#94A3B8' },
};
...
export function rowTint(row, { priorityIdx, doneIdx } = {}) {
  ...
  if (!p) return null;
  return ROW_TINT[p] || ROW_TINT.other;
}
...
// aren't meant to see.
export function buildPriorityStats(rows = [], { priorityIdx, branchIdx, doneIdx }, only = null) {
  if (priorityIdx == null || priorityIdx < 0) return null;
  ...
  for (const row of rows) {
    ...
    if (!PRIORITIES.includes(p) && p !== NO_PRIORITY && !custom.includes(p)) custom.push(p);
  ...
  const used = [...PRIORITIES, ...custom.sort(), NO_PRIORITY].filter(seen);
