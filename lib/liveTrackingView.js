 ...
 */
function branchFromOrderNo(value) {
...
// The Designer department also works across both branches (2026-09-26) — a
// designer's `branch` field still records which office they sit in, but that
// no longer limits which orders' rows they can see.
export function branchScopeFor(user) {
  ...
  if (String(user.department || '').trim().toLowerCase() === 'designer') return null;
...
function sheetSerialToDate(value) {
...
// AppSheet keeps documents in "<Table>_Files_" and photos in "<Table>_Images"
// (no trailing underscore) — the same column mixes both, depending on what
// the user picked in the form. There is no hyperlink on the cell either, so
// the path has to be turned into a Drive file id server-side
// (lib/liveTrackingFiles.js) before anything is clickable. Kept here — with
// no googleapis import — so both sides share it.
const APPSHEET_MARKER = /_(Files_|Images)[/\\]/;
...
export function buildPriorityStats(rows = [], cols = {}, only = null) {
  ...
  return {
    priorities, branches, at, rowTotal, grand,
