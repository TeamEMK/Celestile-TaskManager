 ...
 *
 * A write is: refresh the cached snapshot, mutate the in-memory table, then
 * flush the WHOLE table back to the spreadsheet. There is no row-level locking
 * and no compare-and-swap, because the Sheets API offers neither. Two writes
 * to the same table that overlap in time will therefore have the later flush
 * overwrite the earlier one — a lost update, silently.
 *
 * It is tolerable at this app's write rate (a handful of edits a minute, spread
 * across tables) and it is the price of using a spreadsheet as the database.
 * It is NOT tolerable for anything high-frequency or money-shaped: put those on
 * a real DB_HOST/MySQL deployment instead. makeConnection() below buffers
 * flushes until commit(), which narrows the window for a multi-statement unit
 * of work but does not close it.
 */
