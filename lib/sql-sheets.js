...
async function execute(sql, params) {
  ...
  if ((m = clean.match(/^UPDATE `?(\w+)`?\s+SET\s+(.+?)\s+WHERE\s+(.+)$/i)) ||
    ...
    // assignments (consuming their params) BEFORE parsing WHERE.
    const assigns = [];
    for (const tok of splitTop(setStr)) {
      const am = tok.match(/^`?(\w+)`?\s*=\s*(.+)$/);
      assigns.push({ col: am[1], val: normalizeStore(resolveScalar(am[2], ctx)) });
    }
    const pred = whereStr ? parseWhere(whereStr, ctx) : () => true;
    const targets = arr.filter(pred);
    for (const row of targets) for (const a of assigns) row[a.col] = a.val;
