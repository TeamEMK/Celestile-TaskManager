 ...
 */
import { SCHEMAS, PRIMARY_KEYS, getTables, loadAll, flushTable, appendRows, invalidateCache } from './sheets-client.js';
...
async function execute(sql, params) {
  ...
  if ((m = clean.match(/^INSERT INTO `?(\w+)`?\s*\(([^)]+)\)\s+VALUES\s+(.+)$/i))) {
    ...
    for (const row of newRows) {
      ...
      } else { arr.push(row); appended.push(row); affected += 1; }
    }
    const lastPk = newRows.length ? newRows[newRows.length - 1][PRIMARY_KEYS[table][0]] : null;
    // `appended` is only set when EVERY row was brand new — then the flush can
    // be an append that leaves the rest of the tab alone. A replaced or
    // upserted row needs the full rewrite.
    return {
      rows: [], result: { affectedRows: affected, insertId: lastPk }, dirty: table,
      appended: appended.length === newRows.length ? appended : null,
    };
  }
  ...
  if ((m = clean.match(/^UPDATE `?(\w+)`?\s+SET\s+(.+?)\s+WHERE\s+(.+)$/i)) ||
    ...
    for (const tok of splitTop(setStr)) {
      ...
      const caseM = expr.match(/^CASE\s+WHEN\s+(.+?)\s*=\s*(.+?)\s+THEN\s+(.+?)\s+ELSE\s+(.+?)\s+END$/i);
      if (cm) {
        ...
        // CASE WHEN <a> = <b> THEN <thenExpr> ELSE <elseExpr> END
        const left = resolveScalar(caseM[1], ctx);    // consumes param if '?'
        const right = resolveScalar(caseM[2], ctx);   // consumes param if '?'
        const cond = String(left ?? '') === String(right ?? '');
        const branch = (cond ? caseM[3] : caseM[4]).trim();
        assigns.push({ col, fn: (row) => evalBranch(branch, row) });
      } else {
...
// Evaluate a CASE THEN/ELSE branch: NOW()/CURDATE()/literal/number, or a bare
// column name (referencing the current row).
function evalBranch(expr, row) {
  expr = expr.trim();
  if (/^NOW\(\)$/i.test(expr)) return nowDateTime();
  if (/^CURDATE\(\)$/i.test(expr)) return today();
  if (/^NULL$/i.test(expr)) return null;
  if (expr.startsWith("'")) return stripQuotes(expr);
  if (/^-?\d+(\.\d+)?$/.test(expr)) return Number(expr);
  if (/^`?\w+`?$/.test(expr)) return row[expr.replace(/`/g, '')]; // column ref
  return expr;
}

// store-friendly value: numbers/strings kept; null -> '' ; Date -> iso ;
// boolean -> 1/0 (so e.g. `active` stays numeric, not the string "TRUE")
...
/**
 * CONCURRENCY, read this before adding a write path.
 *
 * A plain INSERT is APPENDED to the tab (sheets-client appendRows): it never
 * touches existing rows, so two processes inserting at once — PM2 runs two
 * cluster instances with separate caches — cannot wipe each other's rows.
 * Daily-task submissions were disappearing exactly that way before.
 *
 * Everything else (UPDATE / DELETE / upsert) is: re-read the sheet if the
 * snapshot is older than REWRITE_MAX_AGE_MS, mutate the in-memory table, then
 * flush the WHOLE table back. There is no row-level locking and no
 * compare-and-swap, because the Sheets API offers neither, so two rewrites of
 * the same table that overlap in time still lose the earlier one. That window
 * is now a few hundred milliseconds rather than the cache TTL, and it is
 * tolerable at this app's edit rate. It is NOT tolerable for anything
 * high-frequency or money-shaped: put those on a real DB_HOST/MySQL deployment
 * instead. makeConnection() below buffers flushes until commit().
 */

// Statements that end in a full-tab rewrite (everything except a plain
// INSERT, which is appended). A rewrite must start from what is on the sheet
// NOW, not from a snapshot up to CACHE_TTL_MS old: the other PM2 instance may
// have appended rows in between, and a stale rewrite deletes them.
const REWRITE_RE = /^\s*(UPDATE|DELETE|REPLACE)\b|ON DUPLICATE KEY UPDATE/i;
const REWRITE_MAX_AGE_MS = 250;

async function flushDirty(table, appended) {
  if (appended) await appendRows(table, appended);
  else await flushTable(table);
}

// pool.query(sql, params) -> [rows|resultHeader, fields]
export async function query(sql, params) {
  await loadAll(false, REWRITE_RE.test(sql) ? REWRITE_MAX_AGE_MS : undefined);
  const r = await execute(sql, params);
  if (r.dirty) await flushDirty(r.dirty, r.appended);
  return [r.result ? r.result : r.rows, undefined];
}

// pool.getConnection() -> transaction-capable connection (buffers flushes)
export function makeConnection() {
  // table -> rows to append, or null once anything in this transaction
  // needs that table fully rewritten.
  const pending = new Map();
  let touched = false;
  return {
    async beginTransaction() { await loadAll(); },
    async query(sql, params) {
      // Same freshness rule as query(), but only while this transaction has
      // not mutated the cache yet — a reload would throw those edits away.
      if (!touched && REWRITE_RE.test(sql)) await loadAll(false, REWRITE_MAX_AGE_MS);
      const r = await execute(sql, params); // operates on the live cache
      if (r.dirty) {
        touched = true;
        const cur = pending.get(r.dirty);
        if (r.appended && cur !== null) pending.set(r.dirty, [...(cur || []), ...r.appended]);
        else pending.set(r.dirty, null);
      }
      return [r.result ? r.result : r.rows, undefined];
    },
    async commit() {
      for (const [t, rows] of pending) await flushDirty(t, rows);
      pending.clear();
    },
    async rollback() { pending.clear(); invalidateCache(); await loadAll(true); },
    release() {},
  };
}
