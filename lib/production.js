...
// Date.now() alone is not unique: seeding eight departments in a loop runs
// inside one millisecond, so every id collided and the rows overwrote each
// other. A per-process counter makes each one distinct.
let idSeq = 0;
function newId(prefix) {
  idSeq = (idSeq + 1) % 1e6;
  return `${prefix}${Date.now().toString(36)}${idSeq.toString(36)}`.toUpperCase().slice(0, 24);
}
...
export function serializeFields(list) {
  ...
  try {
    ...
    if (noteText) {
      await conn.query(
        ...
        [`PN${date}${departmentId}${shift}`.replace(/[^A-Za-z0-9]/g, '').slice(0, 24), date, departmentId, shift, noteText],
      );
    } else {
      await conn.query(
