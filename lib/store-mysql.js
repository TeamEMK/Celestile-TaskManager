...
export async function writeStoreDb(data) {
  ...
  // writeStore() is a whole-store replace: it deletes every users row and
  // re-inserts from the shape readStore() hands back. That shape is a
  // whitelist (see userOut) and deliberately carries no password_hash, access
  // matrix, branch or force-logout stamp — so a plain round trip through
  // read -> write silently logged every user out of their own account and
  // reset their per-page access. Snapshot those columns and put them back.
  let preserved = [];
  try {
    preserved = await q('SELECT id, password_hash, access, branch, force_logout_after FROM users');
  } catch { /* columns may not exist yet on a very old schema */ }
  ...
  try {
    ...
    if (data.users?.length) {
      ...
      // Restore the columns the store shape doesn't carry (see above).
      for (const old of preserved) {
        await conn.query(
          `UPDATE users SET password_hash = ?, access = ?, branch = ?, force_logout_after = ? WHERE id = ?`,
          [old.password_hash ?? null, old.access ?? null, old.branch ?? '', old.force_logout_after ?? null, old.id],
        );
      }
    }

    if (data.delegations?.length) {
      // Every column delegationOut() reads back out (see above) must round
      // trip through here too — this INSERT used to carry only a handful of
      // them, so priority/approval/url/remarks/attachment/etc. were silently
      // dropped on any full-store write (dead in every current deployment,
      // since writeStore() is only reachable with DB_HOST unset — but a
      // landmine for the next feature that calls it, e.g. a restore tool).
      const rows = data.delegations.map((d) => [
        ...
        d.approverId || null, d.approver || '',
        d.url || '', d.remarks || '',
        d.requireFile ? 1 : 0, d.image || '', d.attachment || '',
      ...
      await conn.query(
        `INSERT INTO delegations
          (id,description,doer_id,doer,delegated_by,due_date,client,status,type,
           priority,approval,approver_id,approver,url,remarks,require_file,image,attachment,
           transferred_by,transferred_from,created_at) VALUES ?`,
        [rows]);
    }

    if (data.masters?.length) {
      // Same round-trip fix as delegations above — startDate/requireFile/
      // attachment used to be dropped on write.
      const rows = data.masters.map((m) => [
        m.id, m.task, m.assignedTo || '', m.frequency || 'Daily',
        toMysqlDate(m.startDate), m.requireFile ? 1 : 0, m.attachment || '',
        toMysqlDateTime(m.createdAt) || toMysqlDateTime(new Date().toISOString()),
      ]);
      await conn.query(
        'INSERT INTO masters (id,task,assigned_to,frequency,start_date,require_file,attachment,created_at) VALUES ?',
        [rows]);
    }
