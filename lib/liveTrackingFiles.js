 ...
 *   Live form_Images/09-08-2026 15-12-54.Document Upload.094457.jpg
 *
 ...
 * clickable on its own. The bytes live in a "<Table>_Files_" (documents) or
 * "<Table>_Images" (photos) folder sitting beside the spreadsheet, and the
 * only way to reach them is to look the name up in Drive and serve the file
 * id through /api/drive/<id>.
 *
 * That lookup needs the service account to be able to *see* those folders.
 * Sharing the spreadsheet alone is not enough — Drive shares are per-item, so
 * each folder has to be shared too. When it isn't, everything here degrades to
 * "no link resolved" and the UI falls back to a Drive search link instead of
 * failing — which lands the viewer on drive.google.com, where Google asks
 * them to sign in / request access. The error message names the exact
 * service-account address so whoever owns the sheet knows whom to share with.
 */
...
// Which account the sheet owner has to share the folder with. Only used for
// the error text, so a broken credential setup must not turn a "folder not
// shared" message into a crash.
function serviceAccountEmail() {
  try { return requireGoogleCredentials().client_email || 'unknown'; }
...
async function getIndex(sheetId, folderName) {
  ...
  try {
    ...
    entry = folderId
      ...
      : { at: Date.now(), error: `Drive folder "${folderName}" is not shared with the app's Google service account (${serviceAccountEmail()}).` };
  } catch (err) {
  ...
  try {
    ...
    for (const r of rows || []) {
      ...
      for (const parent of meta.data.parents || []) {
        ...
        for (const f of res.data.files || []) {
          if (/_(Files_|Images)$/.test(f.name || '')) _appFolderIds.add(String(f.id));
        }
      }
    }
  } catch (err) {
