...
export async function requireUser() {
  const { gate } = await requireUserCtx();
  return gate;
}

// Same guard, but hands the session user back too. Handlers that need the
// caller used to run requireUser() and then currentUser() — two full
// getServerSession() decodes (JWT verify + session callback) per request.
// Usage: const { gate, user } = await requireUserCtx(); if (gate) return gate;
export async function requireUserCtx() {
  const user = await currentUser();
  if (!user) return { gate: fail('Unauthorized', 401), user: null };
  if (await suspended()) return { gate: fail('Service suspended', 503), user: null };
  return { gate: null, user };
}
